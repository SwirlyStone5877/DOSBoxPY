...
dosbox_config_path = os.path.join(os.environ['APPDATA'], '..', 'Local', 'DOSBox', 'dosbox-0.74-3.conf')
def restart_script():
...
def modify_settings(submenuvalues, start_line, menu_name):
    ...
    for _ in range(24 - len(submenuvalues)):
        print("")
...
def settings():
    ...
    for dummy in range(22 - len(menuvalues)):
        print("")
...
for _ in range(len(mainmenuvalues)):
    ...
    print(f"{num}: {item}")
for dummy in range(25 - len(mainmenuvalues)):
    print("")

choice = input("Enter your choice: ")
python_exe_path = r'C:\Program Files\Python310\python.exe'
if choice == '1':
    # Launch exe file in "C:\Program Files (x86)\DOSBox-0.74-3\DOSBox.exe" with arguments "-fullscreen -noconsole -userconf"
    dosbox_exe_path = r'"C:\Program Files (x86)\DOSBox-0.74-3\DOSBox.exe"'
    os.system(f'{dosbox_exe_path} -fullscreen -noconsole -userconf')
if choice == '2':
    settings()
if choice == '3':
    # Open the capture directory in Explorer for all users
    capture_directory = os.path.join(os.environ['LOCALAPPDATA'], 'DOSBox', 'capture')
    subprocess.run(['explorer', capture_directory])
    print("Directory opened.")
    time.sleep(2)
    #open python file "__file__" using module subprocess.
    restart_script()
if choice == '4':
    print(f"\nAbout Menu Program\n")
    print("--------------------------------------------------------------------------------------------------------------------")
